---
title: Wasmtime Ships Largest-Ever Security Patch After LLM-Driven Audit Uncovers 12 Vulnerabilities Including Two Critical Sandbox Escapes
date: "2026-04-10T06:38:35.886Z"
tags:
  - "WebAssembly"
  - "Wasmtime"
  - "security"
  ...
  - "open source"
category: News
summary: The Bytecode Alliance patches 12 Wasmtime flaws, two critical, found during a three-week LLM-assisted security sprint by Mozilla, UCSD, Akamai, and F5.
sources:
...
provenance_id: 2026-04/10-wasmtime-ships-largest-ever-security-patch-after-llm-driven-audit-uncovers-12-vulnerabilities-including-two-critical-sandbox-escapes
...
The Bytecode Alliance on April 9 released patch versions of its Wasmtime WebAssembly runtime addressing 12 distinct security vulnerabilities, two of them rated Critical. The disclosure represents the [largest batch of security advisories](https://bytecodealliance.org/articles/wasmtime-security-advisories) the project has ever published at once — triple the total number of advisories issued in all of 2025 and double the number of Critical-severity findings in the runtime's history.
...
Patched versions — [Wasmtime 43.0.1, 42.0.2, 36.0.7, and 24.0.7](https://github.com/bytecodealliance/wasmtime/releases/tag/v43.0.1) — were released simultaneously on April 9. The fixes span four supported release branches, reflecting the breadth of affected code.

The two Critical-severity issues are both sandbox escapes. CVE-2026-34987 affects the Winch compiler backend and could allow a WebAssembly guest module to perform out-of-sandbox memory access. CVE-2026-34971 stems from a miscompilation of guest heap accesses in the Cranelift code generator on AArch64, a bug [introduced in Wasmtime 32.0.0](https://bytecodealliance.org/articles/wasmtime-security-advisories). Users running Cranelift on x86-64 are not affected by either Critical-severity escape.
...
- **Runtime**: a use-after-free in `wasmtime::Linker` cloning, and [data leakage between pooling allocator instances](https://nvd.nist.gov/vuln/detail/CVE-2026-27204)

### The LLM Audit

Shun Kashiwa from UCSD developed the multi-agent harness and the LLM prompts that uncovered the majority of the issues, according to the [Bytecode Alliance's disclosure](https://bytecodealliance.org/articles/wasmtime-security-advisories). The harness targeted Wasmtime's most security-sensitive code paths — native code generation in the Cranelift and Winch engines, and `unsafe` Rust in the runtime — across different architectures and backends, then validated each potential bug by reproducing proof-of-concept exploits and test vectors.
...
It remains unclear how broadly AArch64-based Wasmtime deployments are used in production. Akamai, which [acquired Fermyon in late 2025](/article/2026-02/21-wasi-03-brings-native-async-io-to-webassembly-as-akamais-fermyon-acquisition-signals-server-side-inflection-point) to run WebAssembly workloads across its edge network, was among the organizations involved in the audit — suggesting that the findings may have direct implications for large-scale edge deployments.
